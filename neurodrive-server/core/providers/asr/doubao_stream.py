...
class ASRProvider(ASRProviderBase):
    ...
    async def receive_audio(self, conn, audio, audio_have_voice):
        ...
        if audio_have_voice:
            self.prev_have_voice = True
        else:
            if self.asr_ws and self.is_processing and self.prev_have_voice and getattr(conn, 'client_voice_stop', False):
                try:
                    audio_request = bytearray(self.generate_last_audio_default_header())
                    audio_request.extend((0).to_bytes(4, 'big'))
                    await self.asr_ws.send(audio_request)
                except Exception as e:
                    logger.bind(tag=TAG).info(f"发送结束音频标识时发生错误: {e}")
                finally:
                    self.prev_have_voice = False
                    ...
                    if "payload_msg" in result:
                        ...
                        if "result" in payload:
                            ...
                            if (
                                ...
                                if len(audio_data) >= 10:  # 确保有足够音频数据
