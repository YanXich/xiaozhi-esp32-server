...
class CacheConfig:
    ...
    def for_type(cls, cache_type: CacheType) -> "CacheConfig":
        ...
        configs = {
            ...
            CacheType.WAKEUP_WORDS: cls(
                strategy=CacheStrategy.TTL, ttl=300, max_size=500  # 5分钟过期
            ),
             # 新增：设备信息缓存，默认手动失效（由设备回传更新）
            CacheType.DEVICE_INFO: cls(
                strategy=CacheStrategy.TTL, ttl=None, max_size=2000  # 手动失效
            ),
        }
