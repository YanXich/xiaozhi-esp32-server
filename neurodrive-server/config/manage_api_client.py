...
def get_car_wakeup_word(device_id: str) -> Optional[str]:
    """通过车载设备ID获取唤醒词（car_name）"""
    try:
        # 调用车载设备API
        with httpx.Client(timeout=10.0) as client:
            response = client.post(
                "https://api.neurodrive.cn/api/car/getInfo",
                json={"device_mac": device_id},
            ...
            if response.status_code == 200:
                ...
                if data.get("code") == 0 and data.get("data"):
                    car_name = data["data"].get("car_name")
                    if car_name:
                        print(f"获取到车载唤醒词: {car_name}")
                        return car_name
                    else:
                        print("车载设备信息中未找到car_name")
                else:
                    print(f"车载设备API返回错误: {data.get('message', '未知错误')}")
            else:
                print(f"车载设备API请求失败，状态码: {response.status_code}")
