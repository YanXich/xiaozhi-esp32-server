...
CREATE TABLE device_factory (
  ...
  status tinyint NOT NULL COMMENT '状态(0:启用, 1:停用)',
  PRIMARY KEY (id)
...
CREATE TABLE device_production_batch (
  ...
  status tinyint NOT NULL COMMENT '状态(0:待生产, 1:生产中, 2:已生产, 3:作废)',
...
CREATE TABLE device_number (
  ...
  status tinyint NOT NULL COMMENT '状态(0:初始化, 1:已生成, 2:已绑定)',
  create_date datetime COMMENT '创建时间',
  update_date datetime COMMENT '更新时间',
  PRIMARY KEY (id)
...
CREATE TABLE device_mac (
  ...
  status tinyint NOT NULL COMMENT '状态(0:未使用, 1:已使用)',
  create_date datetime COMMENT '创建时间',
  update_date datetime COMMENT '更新时间',
  PRIMARY KEY (id)
