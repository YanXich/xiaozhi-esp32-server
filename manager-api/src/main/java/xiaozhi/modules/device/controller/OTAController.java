...
public class OTAController {
    ...
    @Operation(summary = "OTA获取车型控制命令")
    @GetMapping("/car-command/{macAddress}")
    public ResponseEntity<String> checkCarCommand(@PathVariable String macAddress) {
        ...
        //获取设备的指令配置
        try {
            String result = deviceMacService.getDeviceConfigByMac(macAddress);
            return  ResponseEntity.ok(result);
        } catch (RuntimeException e) {
            return createResponse(DeviceReportRespDTO.createError(e.getMessage()));
        }
    }
